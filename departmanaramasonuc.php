  ...
  <? menu();?>
    ...
    <div class="content-wrapper">
		...
		<? if ($_POST){
						...
						$ara=$_POST["arama"];
						if($ara == null){
							echo "Hata simgesi verin";
							
						}
						else {
						
						$v = $db-> prepare("select * from department where departmentName like ?");
						$v -> execute(array('%'.$ara.'%'));
	...
	<button type="button" class="btn btn-primary waves-effect waves-light" data-toggle="tooltip" data-placement="top" title="" >
                                    <i class="icofont icofont-home "></i><span class="m-l-10"><a href="index.php"><font color="white">Ana Sayfa</font></a></span>
                                </button>
						</div>
					</div>
				</div>
